...
test.describe('Phase 6D contextual gate presentation and continuation', () => {
  ...
  test('exposes only canonical debug scenarios, resets to normal L2, and rejects unknown names', async ({ page }) => {
    const result = await page.evaluate(() => {
      ...
      const reset = JSON.parse(localStorage.getItem('campushub:state:v3:tenant-makerere:membership-demo-001'));
      window.CampusHubDebug.setScenario('voice-under-review');
      return { snapshots, error, reset, voiceStatus:JSON.parse(localStorage.getItem('campushub:state:v3:tenant-makerere:membership-demo-001')).voiceStatusScenario };
    });
