...
test.describe('Phase 8J canonical Event detail', () => {
  ...
  test('preserves RSVP Going, Interested, clear, zero XP, and tenant-day streak behavior', async ({ page }) => {
    ...
    const startingStreak = await page.evaluate(() => JSON.parse(localStorage.getItem('campushub:state')).streakState.count);
    ...
    expect(await page.evaluate(() => JSON.parse(localStorage.getItem('campushub:state')).rsvp)).toBe('going');
    expect(await page.evaluate(() => window.CampusHubDemo.student.xp)).toBe(startingXp);
    const afterGoingStreak = await page.evaluate(() => JSON.parse(localStorage.getItem('campushub:state')).streakState.count);
    ...
    expect(await page.evaluate(() => JSON.parse(localStorage.getItem('campushub:state')).rsvp)).toBe(null);
    expect(await page.evaluate(() => JSON.parse(localStorage.getItem('campushub:state')).streakState.count)).toBe(afterGoingStreak);
    expect(await page.evaluate(() => window.CampusHubDemo.student.xp)).toBe(startingXp);
    ...
    expect(await page.evaluate(() => JSON.parse(localStorage.getItem('campushub:state')).rsvp)).toBe('interested');
    expect(await page.evaluate(() => window.CampusHubDemo.student.xp)).toBe(startingXp);
  });
